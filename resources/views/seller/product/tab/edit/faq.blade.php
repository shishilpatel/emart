  <div class="panel-heading">
      <a data-toggle="modal" data-target="#addFAQ" class="btn btn-primary mb-3"><i class="fa fa-plus-circle"></i> Add FAQ</a> 
              ...
              <td>
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu" aria-labelledby="CustomdropdownMenuButton3">
                        <a class="dropdown-item"   title="Edit FAQ" data-toggle="modal" data-target="#editfaq{{ $f->id }}"><i class="feather icon-edit mr-2"></i>{{ __("Edit")}}</a>
                        <a class="dropdown-item"   data-toggle="modal" data-target="#faqdel{{ $f->id }}"><i class="feather icon-delete mr-2"></i>{{ __("Delete")}}</a>
      ...
      @foreach($faqs as $key => $f)
        ...
        <div class="modal fade" id="editfaq{{ $f->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5  class="modal-title">Edit FAQ: {{ $f->question }}</h5>
              ...
              <form id="demo-form2" method="post" action="{{route('product_faq.update',$f->id)}}">
                {{ method_field("PUT") }}
                  ...
                  <div class="form-group">
                    ...
                    <textarea required="" cols="10" id="editor1" name="answer" rows="5" class="form-control editor1">{{ $f->answer }}</textarea>
